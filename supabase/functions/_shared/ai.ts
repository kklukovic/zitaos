...
export type CallAIOpts = {
  ...
  webSearch?: boolean;  // ground the response with live Google Search (Gemini only)
...
function dispatch(provider: Provider, opts: CallAIOpts): Promise<AIResult> {
  switch (provider) {
    ...
    case "anthropic":
      if (opts.webSearch) {
        throw new Error("Anthropic fallback has no web-search grounding; refusing ungrounded research");
      }
...
async function callGemini(opts: CallAIOpts): Promise<AIResult> {
  ...
  if (opts.jsonMode && !opts.webSearch) genConfig.responseMimeType = "application/json";
  ...
  if (opts.webSearch) requestBody.tools = [{ google_search: {} }];
  ...
  const groundingSources: GroundingSource[] = (groundingMetadata?.groundingChunks ?? [])
    ...
    .filter((source: GroundingSource) => source.title && source.url);
